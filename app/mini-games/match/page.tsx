...
const initialCards = cardImages.flatMap(img => [img, img]).sort(() => Math.random() - 0.5)
    ...
    if (newSelectedCards.length === 2) {
      ...
      if (first.card === second.card) {
        setMatchedCards(prev => {
          const newMatches = [...prev, first.card]
              ...
              {cards.map((card, index) => (
                ...
                >
                  <CardContent className="p-0">
                    {selectedCards.some(sc => sc.index === index) || matchedCards.includes(card) ? (
                      <Image
                        src={card}
                        alt="Card"
                        width={100}
